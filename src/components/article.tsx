...
export const Article = ({item, onPressCheckbox, onSwipeDelete}:ArticleProps):JSX.Element => {
    ...
    const renderCheckAction = (isCheked:boolean) => {
        ...
        return <List.Item title={''} style={styles.checkSwipeList}
                    left={props => <List.Icon {...props} color={Colors.white} icon={icon} />} />
    }
        ...
        <> 
            ...
            <Swipeable useNativeAnimations 
                ...
                <List.Item title={item.name} 
                    key={key}
                    right={props => <Checkbox {...props} status={status} onPress={()=> onPressCheckbox(item.id, !item.checked)} />}
                    style={[item.checked ? styles.checkedArticle : styles.uncheckedArticle]} 
